...
export const BackupEmailPage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    if (trimmedEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmedEmail)) {
      try { // @ts-ignore
        (window as any)?.Telegram?.WebApp?.showAlert?.('Please enter a valid email address.');
      } catch { 
    ...
    try {
      ...
      try { // @ts-ignore
        (window as any)?.Telegram?.WebApp?.showPopup?.({ 
          title: 'Backup Email', 
          message: trimmedEmail ? 'Email saved successfully.' : 'Email removed successfully.' 
        });
      } catch {}
      ...
      try { // @ts-ignore
        (window as any)?.Telegram?.WebApp?.showAlert?.('Failed to save backup email. Please try again.');
      } catch { 
