...
export const PasscodePage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (!passcodeEnabled) {
        if (!newPasscode || newPasscode.length !== 6 || newPasscode !== confirmPasscode) { setBusy(false); return; }
        await api.updatePreferences({ setPasscode: { newPasscode } });
      } else {
        if (!currentPasscode || currentPasscode.length !== 6) { setBusy(false); try { (window as any)?.Telegram?.WebApp?.showAlert?.('Please enter your current passcode.'); } catch { alert('Please enter your current passcode.'); } return; }
        if (!newPasscode || newPasscode.length !== 6 || newPasscode !== confirmPasscode) { setBusy(false); return; }
        await api.updatePreferences({ setPasscode: { currentPasscode: currentPasscode || undefined, newPasscode } });
      }
      navigate('/settings');
    } catch (e) {
      try { // @ts-ignore
        (window as any)?.Telegram?.WebApp?.showAlert?.(String((e as any)?.message || 'Failed to update passcode'));
      } catch { alert(String((e as any)?.message || 'Failed to update passcode')); }
    } finally {
      setBusy(false);
    }
  };
  ...
  const handleDisable = async () => {
    if (busy || !passcodeEnabled) { navigate('/settings'); return; }
  ...
  return (
      ...
      <div className="px-4 mt-6 relative z-10">
        <div className="form-card w-full max-w-[408px]">
              ...
              <div className="self-stretch flex flex-col justify-start items-start gap-3">
                ...
                {passcodeEnabled && (
                  <div className="self-stretch inline-flex flex-col gap-2">
                    <div className="text-neutral-400 text-sm">Current passcode</div>
                    <PinInput length={6} value={currentPasscode} onChange={setCurrentPasscode} disabled={busy} />
                  </div>
                )}

                <div className="self-stretch inline-flex flex-col gap-2">
                  <div className="text-neutral-400 text-sm">{passcodeEnabled ? 'New passcode' : 'Passcode'}</div>
                  <PinInput length={6} value={newPasscode} onChange={setNewPasscode} disabled={busy} autoFocus={!passcodeEnabled} />
                </div>
                <div className="self-stretch inline-flex flex-col gap-2">
                  <div className="text-neutral-400 text-sm">Re-type passcode</div>
                  <PinInput length={6} value={confirmPasscode} onChange={setConfirmPasscode} disabled={busy} />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="self-stretch inline-flex justify-between items-center gap-4">
            ...
            <button
              onClick={handleSave}
              disabled={busy || !newPasscode || newPasscode.length !== 6 || newPasscode !== confirmPasscode || (passcodeEnabled && (!currentPasscode || currentPasscode.length !== 6))}
    ...
    {showDisableModal && (
      <PasscodePromptModal
        ...
        onCancel={() => { setShowDisableModal(false); setDisableError(null); }}
        onSubmit={async ({ currentPasscode }) => {
          try {
            await api.updatePreferences({ disablePasscode: { currentPasscode } });
