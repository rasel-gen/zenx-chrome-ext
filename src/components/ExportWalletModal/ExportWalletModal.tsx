...
export const ExportWalletModal: React.FC<ExportWalletModalProps> = ({
  ...
  const handleExport = async () => {
    ...
    setLoading(true)
    try {
      const phrase = await useWalletStore
        .getState()
        .exportKeyring(keyringId, passcode || undefined)
    ...
    ;(async () => {
      ...
      try {
        const prefs = await api
          .preferences()
          .catch(() => ({ passcodeEnabled: false }) as any)
        ...
        if (!required && keyringId) {
          ...
          try {
            const phrase = await useWalletStore
              .getState()
              .exportKeyring(keyringId, undefined)
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="text-center">
              <p className="text-neutral-400 text-sm font-['Manrope']">
                Store this recovery phrase in a safe place. Anyone with this
    ...
    return (
      <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
        <div className="w-full max-w-sm bg-zinc-950 rounded-2xl border border-[#25282F] p-6 shadow-2xl flex flex-col items-center gap-4">
          ...
          <div className="text-neutral-400 text-sm font-['Manrope']">
            Preparing export...
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
            <p className="text-red-200 text-sm font-['Manrope'] leading-relaxed">
              <strong>Warning:</strong> Your recovery phrase gives full access
              to your wallet. Never share it with anyone or store it online.
            </p>
          </div>
