  ...
  const items = useMemo(() => {
    const fmt = (timestamp: Date) => {
      if (!timestamp || isNaN(timestamp.getTime()))
        return { date: "", time: "" }
      const dd = timestamp.toLocaleDateString(undefined, {
        day: "2-digit",
        month: "2-digit",
        year: "2-digit"
      })
      const tt = timestamp.toLocaleTimeString(undefined, {
        hour: "numeric",
        minute: "2-digit"
      })
      ...
      <div className="px-4 pb-28">
        <div className="w-full inline-flex flex-col gap-2">
          {!loading && items.length === 0 && (
            <div className="text-neutral-400 text-sm px-2 py-6 text-center">
              No transactions yet
            </div>
          )}
          {items.map((item) => (
            <button
              onClick={() =>
                navigate(`/transaction/${item.id}`, { state: item.raw })
              }
              key={item.id}
              className="text-left self-stretch px-4 py-3 bg-neutral-900 rounded-2xl outline outline-1 outline-offset-[-1px] outline-zinc-800 flex flex-col gap-2.5">
              <div className="self-stretch inline-flex justify-between items-start">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="inline-flex flex-col items-start gap-1 min-w-0">
                    <div className="text-gray-100 text-base font-medium leading-snug truncate max-w-[65vw]">
                      {item.title}
                    </div>
                    <div className="text-neutral-400 text-sm font-medium leading-tight truncate max-w-[60vw]">
                      {item.desc}
                    </div>
                  </div>
                </div>
                <div className="inline-flex flex-col items-end gap-1 shrink-0">
                  <div className="text-right text-neutral-400 text-xs sm:text-sm font-medium leading-tight whitespace-nowrap">
                    {item.date}
                  </div>
                  <div className="text-right text-neutral-400 text-xs sm:text-sm font-medium leading-tight whitespace-nowrap">
                    {item.time}
