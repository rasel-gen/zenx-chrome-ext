...
import { SendAssetSheet } from '@/components/SendAssetSheet/SendAssetSheet.tsx';
        ...
        {assets.map((a) => (
          ...
          >
            <div className="flex items-center gap-3">
              <img src={a.icon} alt={a.name} className="w-10 h-10 rounded-full" />
            ...
            <div className="w-36 flex justify-between items-center">
              <div className="inline-flex flex-col items-start gap-2">
                <div className="text-gray-100 text-base font-medium leading-snug">{a.balance.toFixed(4)} {a.symbol}</div>
                <div className="text-gray-100 text-base font-medium leading-snug">${a.balanceUSD.toFixed(2)}</div>
              </div>
              <div className="w-10 h-10 bg-neutral-800 rounded-[50px] outline outline-1 outline-offset-[-1px] outline-zinc-800 flex justify-center items-center">
                <img src="/arrow-send.png" alt="" className="w-5 h-5 brightness-0 invert opacity-80" />
              </div>
            </div>
          </button>
        ))}
