...
export const CurrencyBadge: React.FC<Props> = ({ code, size = 24, className }) => {
  ...
  return (
    ...
    >
      {fallback.type === 'icon' ? (
        <img src={fallback.value} alt={`${code} icon`} style={{ width: size, height: size }} className="object-cover" />
      ) : Icon ? (
        <Icon size={Math.max(12, Math.floor(size * 0.7))} className="text-gray-200" />
      ) : (
        <span className="text-gray-200 font-bold" style={{ fontSize: Math.max(9, Math.floor(size * 0.35)) }}>
          {fallback.value}
        </span>
      )}
    </div>
