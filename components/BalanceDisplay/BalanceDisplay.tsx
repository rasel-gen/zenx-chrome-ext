...
export const BalanceDisplay: React.FC<BalanceDisplayProps> = ({
  ...
  const [intPart, decPart] = formatBalance(balance).split('.') as [string, string?];
      ...
      <div className="self-stretch flex flex-col items-start gap-6">
        <div className="self-stretch inline-flex justify-between items-start">
          <div className="inline-flex flex-col items-start gap-3">
            <div className="text-neutral-400 text-base font-normal leading-normal">Current Balance</div>
            <div className="inline-flex items-center gap-3">
              <div className="justify-center">
                {isVisible ? (
                  <>
                    <span className="text-gray-100 text-3xl font-bold leading-9 tracking-wider">{FIAT_SYMBOLS[currency] ?? ''}{intPart}</span>
                    <span className="text-neutral-400 text-3xl font-medium leading-9 tracking-wider">.{decPart ?? '00'}</span>
                  </>
                ) : (
                  <span className="text-gray-100 text-3xl font-bold leading-9 tracking-wider">••••••</span>
                )}
              </div>
              <button
                ...
                style={{ background: 'transparent', color: 'var(--color-hint)' }}
              >
          ...
          <div className="pl-1 pr-2 py-1 bg-slate-800/10 rounded-[50px] outline outline-1 outline-offset-[-1px] outline-zinc-700 flex items-center gap-3 relative">
            ...
            >
              ...
              <div className="text-neutral-400 text-sm font-normal leading-tight">{currency}</div>
            </button>
            <div className="w-5 h-5 flex justify-center items-center">
              <img src="/arrow-down-01.png" alt="Dropdown arrow" className="w-4 h-4 opacity-60" />
            </div>
              ...
              <>
                ...
                >
                  <div className="self-stretch flex flex-col gap-4">
                    <div className="self-stretch inline-flex justify-between items-start">
                      <div className="text-gray-100 text-base leading-normal">Base Currency</div>
                      <button
                    ...
                    <div className="self-stretch flex flex-col overflow-y-auto pr-1" style={{ maxHeight: '60vh' }}>
                      {currencies.map((code) => (
                        <button
                          key={code}
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="inline-flex flex-col items-start">
                              <div className="text-gray-100 text-base font-semibold leading-snug">{code}</div>
                              <div className="text-neutral-400 text-sm font-medium leading-tight">{FIAT_META[code]?.label ?? ''}</div>
                            </div>
                          </div>
        ...
        <div className="self-stretch flex flex-wrap gap-2 justify-center sm:justify-between items-center">
          {[
            { key: 'send', label: 'Send', icon: '/arrow-send.png', onClick: onSend },
            { key: 'receive', label: 'Receive', icon: '/arrow-receive.png', onClick: onReceive },
            { key: 'history', label: 'History', icon: '/transaction-history.png', onClick: onHistory },
          ].map((btn) => (
            <button
              key={btn.key}
              onClick={btn.onClick}
            ...
            >
              <div className="w-8 h-8 sm:w-10 sm:h-10 bg-zinc-800 rounded-[60px] flex justify-center items-center flex-shrink-0">
                <img src={btn.icon} alt="" className="w-4 h-4 sm:w-5 sm:h-5 brightness-0 invert" />
              </div>
              <div className="text-slate-200 text-sm sm:text-base font-medium leading-normal truncate">{btn.label}</div>
            </button>
          ))}
        </div>
      </div>
